...
export default function QuestionsRunner({
  ...
  useEffect(() => {
    ...
    if (deadline - Date.now() <= 0 && !submittedOnExpire.current) {
      ...
      const hasCorrectLocal = questions.every(
      ...
      const payload: SubmitPayload = {
        picked,
        score: hasCorrectLocal ? c : undefined,
        total: hasCorrectLocal ? total : undefined,
      };
      onSubmit?.(payload);
      setShowResults(true);
    }
  ...
  //  Navigation
  if (!hydrated) return <div className="px-6 mx-auto" />;
