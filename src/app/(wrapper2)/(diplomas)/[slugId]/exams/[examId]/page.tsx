...
export default async function ExamQuestionsPage({
  ...
  const res = await fetch(
    `https://exam-app-back-iota.vercel.app/questions?exam=${examId}`,
    {
  ...
  const trackTitle = first?.subject?.name
    ? `${first.subject.name} - ${examTitle}`
    : examTitle;
      ...
      <HeaderBar
        title={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="sm:ml-2">Questions</span>
          </div>
        }
        icon={
          <HelpCircle className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12" />
        }
      />
